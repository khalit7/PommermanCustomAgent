...
public class RollOutBiasHeuristic extends CustomStateHeuristic {
    private players.PommermanCustomAgent.heuristics.RollOutBiasHeuristic.BoardStats rootBoardStats;

    public RollOutBiasHeuristic(GameState root) {
        rootBoardStats = new players.PommermanCustomAgent.heuristics.RollOutBiasHeuristic.BoardStats(root);
    }

    @Override
    public double evaluateState(GameState gs,int obj) {
        boolean gameOver = gs.isTerminal();
        Types.RESULT win = gs.winner();

        // Compute a score relative to the root's state.
        players.PommermanCustomAgent.heuristics.RollOutBiasHeuristic.BoardStats lastBoardState = new players.PommermanCustomAgent.heuristics.RollOutBiasHeuristic.BoardStats(gs);
    ...
    public static class BoardStats
    {
         ...
         * @return a score [0, 1]
         */
        double score(players.PommermanCustomAgent.heuristics.RollOutBiasHeuristic.BoardStats futureState)
        {
